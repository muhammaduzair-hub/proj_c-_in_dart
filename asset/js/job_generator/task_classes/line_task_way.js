...
/* global Infinity, RobotAction, range */

class LineTaskWay extends Task
{
  constructor( id, ends, reverse, paint )
  {
    var name = "way";
    super( id, name, ends, reverse, paint );
  ...
  copy()
  {
    var newTask = new LineTaskWay( this.id, this.ends.copy(), this.reverse, this.paint );
  ...
  splitPercent( procent )
  {
    ...
    var split_end = this.ends[split_index + 1].copy();
  ...
  sample( sample_length = 0.01, return_lines = false)
  {
    ...
    for( var i = 0; i < this.ends.length - 1; i++ )
    {
      ...
      sample_ends.push( single_line_samples[j].end );
    }
    ...
    return new LineTaskWay( this.id, sample_ends, this.reverse, this.paint );
  }
  ...
  {
    return new LineTaskWay( this.id, this.ends.reverse(), this.reverse, this.paint );
  }
  ...
  exportTask()
  {
    return new WaypointTask( this.id, this.ends[0], this.reverse, this.paint ).exportTask();
  }
}