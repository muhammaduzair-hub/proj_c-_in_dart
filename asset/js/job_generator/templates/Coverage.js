...
class Coverage extends Job
{
  static template_type = "Coverage"; // Translateable
  static template_title = "Standard"; // Translateable
  static template_id = "coverage"; // no spaces
  ...
  constructor( id, name, layout_method )
  {
    ...
    this.options.align_dots = {
      get configurable()
      {
        return this_class.options.make_dots.val || this_class.options.make_dash.val;
      },
      get name()
      {
  ...
  make_task( ends )
  {
    if( this.options.DrawAsPoints.val )
      this.tasks.push( new LineTaskWay( this.tasks.length, ends, false, true ) );
    else
  ...
  draw()
  {
            ...
            if( first && c2 )
            {
              ...
              if( this.tasks.length )
              {
                ...
                best_path.forEach( ( p ) => {
                  ...
                  new_task.task_options.push( new BoolRobotAction( "coarse_line_start", true ) );
    ...
    if( !this.options.OptimizeRoute.val && this.options.LineSpacing.val > 0 )
    {
      // Create all lines
      var oposit = 0;
      for( var y = limits[2] + this.options.LineSpacing.val * 0; y < limits[3]; y += this.options.LineSpacing.val )
      {
        var l = new Line( new Vector( limits[0], y ), new Vector( limits[1], y ) );
        l = l.add_to_end( 10 ).add_to_start( 10 );
        var crosses = [ ];
      ...
      // optimize route
      var rows = this.tasks.splice( 0 );
      rows.forEach( ( r, i ) => {
        r.id = i;
      } );
      var last_task = rows.shift();
      if( last_task )
        this.tasks.push( last_task );

      if( !this.options.OptimizeRoute.val )
      {
        while( rows.length )
        {
          var next_task = rows.shift();
          var best_path = [ ];
          if( border_map[last_task.to_border].indexOf( next_task.from_border ) === -1 )
          {
            var route = this.route_between_points( last_task.end, next_task.start, border_lines, points, point_map, longest_dist );
            best_path = route[1].slice( 0, -1 );
      ...
      else
      {
        ...
        while( rows.length )
        {
          ...
          var check = ( t, i ) => {
            ...
            let p_start = this.route_between_points( last_task.end, t.start, border_lines, points, point_map, longest_dist );
            let p_end = this.route_between_points( last_task.end, t.end, border_lines, points, point_map, longest_dist );
            let d_start = p_start[0];
            let d_end = p_end[0];
            if( d_start < min_d )
            {
              min_d = d_start;
          ...
          var next_task = rows.splice( min_d_index, 1 )[0];
          if( !min_d_is_start )
            next_task = next_task.opposite_direction;
          next_task.id = this.tasks.length;
          best_path = best_path.slice( 0, -1 );
          next_task.best_path = best_path;
    ...
    if( this.options.make_dots.val || this.options.make_dash.val )
    {
      if( this.options.dot_distance.val > 0 )
      {
        ...
        this.tasks = this.tasks.map( lt => {
          ...
          if( this.options.align_dots.val )
            offset += ref_x - lt.start.x;
          ...
          if( this.options.make_dots.val )
          {
            let wp = [ ];
            var splittet = lt.toLine().split( 0.001, this.options.dot_distance.val - 0.001, offset );
            splittet.forEach( part => {
              //if( part.length.veryclose( this.options.dot_distance.val / 2 ) )
              wp.push( new WaypointTask( this.tasks.length, part.start, false, true ) );
            } );
            if( oposit )
              wp = wp.reverse();
            return wp;
